...
import { NavigationMenu, type NavItem } from '@/components/navigation/NavigationMenu';
...
export function AppLayout() {
  ...
  const navItems: NavItem[] = useMemo(
    () => [
      { id: 'home', to: '/', end: true, label: t('nav.home'), icon: <Home size={18} aria-hidden /> },
      {
        id: 'dashboard',
        to: '/dashboard',
  ...
  return (
    <div className={styles.shell}>
      ...
      <Drawer open={mobileOpen} onClose={() => setMobileOpen(false)} title={t('nav.menu')} side="start">
            ...
            {navItems.map((item) => (
              <li key={item.id}>
                <NavLink
                  ...
                  className={({ isActive }) => cn(styles.mobileLink, isActive && styles.mobileActive)}
